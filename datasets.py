...
def get_dataset(dataset, download=False, seen=None, train=False):
    if 'MNIST' in dataset:
        ...
        train_transform, test_transform = tinyImageNet_transform()
    else:
    ...
    if dataset == 'cifar10':
        ...
        test_dir = os.path.join('/home/linhw/myproject/CSI/data', 'LSUN_resize')
        test_set = datasets.ImageFolder(test_dir, transform=transforms.ToTensor())

    elif dataset == 'cifar10_interp':
        test_dir = '/home/linhw/data/cifar10_interp'
        test_set = datasets.ImageFolder(test_dir, transform=transforms.ToTensor())

    elif dataset == 'ImageNet':
        test_dir = os.path.join('/home/linhw/myproject/CSI/data', 'Imagenet_resize')
        test_set = datasets.ImageFolder(test_dir, transform=transforms.ToTensor())
    
    elif dataset == 'LSUN-FIX':
        test_dir = os.path.join('/home/linhw/myproject/CSI/data', 'LSUN_fix')
        test_set = datasets.ImageFolder(test_dir, transform=transforms.ToTensor())
    
    elif dataset == 'ImageNet-FIX':
        test_dir = os.path.join('/home/linhw/myproject/CSI/data', 'Imagenet_fix')
        test_set = datasets.ImageFolder(test_dir, transform=transforms.ToTensor())
    
    elif dataset == 'CIFAR100':
        test_dir = '/home/linhw/cifar100_data'
        test_set = datasets.CIFAR100(test_dir, train=False, transform=transforms.ToTensor())
        ...
        test_class_idx = [i for i in range(len(seen))]
        train_set = my_get_subclass_dataset(train_set, test_class_idx)
        test_set = my_get_subclass_dataset(test_set, test_class_idx)
